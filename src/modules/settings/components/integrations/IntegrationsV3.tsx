...
import {
  ChevronRight, Search, Layers, LayoutGrid, Database, Zap, Cpu, Plus,
  X,
} from "lucide-react";
...
const tabIcons: Record<TabId, React.FC<{ size: number; color: string }>> = { all: Layers, apps: LayoutGrid, dataSources: Database, services: Zap, models: Cpu };

const categoryLabel: Record<CategoryId, string> = { apps: "App", dataSources: "Data", services: "Service", models: "Model" };
  ...
  return (
    <div style={{ fontFamily: f }}>
      ...
      <div style={{ display: "flex", gap: 6, borderBottom: `1px solid ${ws.divider}`, paddingBottom: 8, marginBottom: 16 }}>
        {[40, 70, 50, 65, 60].map((w, i) => <ShimmerBar key={i} width={w} height={28} delay={i * 60} />)}
      </div>
...
export default function IntegrationsV3() {
  ...
  const isCategoryTab = !isAllTab && !isModelsTab;

  /* ── Unified flat list ────────────────────────────────────────── */
  const allIntegrationItems: IntegrationItem[] = [...allApps, ...allDataSources, ...allServices];

  /* Integration data — tab-specific or all */
  const tabItems = isCategoryTab
    ? (activeTab === "apps" ? allApps : activeTab === "dataSources" ? allDataSources : allServices)
    : allIntegrationItems;
  ...
  const enabledItems = tabItems.filter((i) => i.enabled && searchFilter(i));
  const availableItems = tabItems.filter((i) => !i.enabled && searchFilter(i));
  const enabledLabel = activeTab === "apps" ? "Enabled" : "Connected";

  /* Model filtering (for All tab and Models tab) */
  const showModelsSection = isAllTab || isModelsTab;
  const filteredModelConfigured = showModelsSection ? [...configuredProviders, ...selfHostedConfigured].filter(searchFilter) : [];
  const filteredModelAvailable = showModelsSection ? availableProviders.filter(searchFilter) : [];

  const totalCount = allIntegrationItems.length + availableProviders.length + configuredProviders.length + selfHostedConfigured.length + 1;
    ...
    return (
      <ListRow key={item.id} onClick={() => handleSelectItem(item.id)} selected={isSelected} last={last}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          ...
          {isAllTab && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: ws.elevated, color: ws.secondary, fontFamily: f, flexShrink: 0 }}>{categoryLabel[item.category]}</span>}
        </div>
      ...
      <ListRow key={item.id} onClick={() => handleSelectItem(item.id)} selected={isSelected} last={last}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          {item.configured && isGenie && (
            <span style={{ fontSize: 11, color: ws.muted_text, fontFamily: f }}>{(configuredProviders.length > 0 || selfHostedConfigured.length > 0) ? "Safety net \u00b7 active" : "Auto-selects best model"}</span>
          )}
          {!item.configured && (
  ...
  return (
    <>
      ...
      <div ref={panelAnchorRef} style={{ fontFamily: f }}>
        ...
        <div style={{ display: "flex" }}>
          ...
          <div style={{
            ...
            {/* Search bar — only in All tab, below tabs */}
            {isAllTab && (
            ...
            {isAllTab && (
              <>
                ...
                {(filteredModelConfigured.length > 0) && (
                  <div style={{ marginBottom: 24 }}>
                    <SectionLabel>Models</SectionLabel>
                    <Card>
                      {renderModelRow(genieManaged, filteredModelConfigured.length === 0)}
                      {filteredModelConfigured.map((item, i) => renderModelRow(item, i === filteredModelConfigured.length - 1))}
                    </Card>
                  </div>
                )}
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <Card>
                    {availableItems.map((item, i) => renderRow(item, i === availableItems.length - 1 && filteredModelAvailable.length === 0))}
                    {filteredModelAvailable.map((item, i) => renderModelRow(item, i === filteredModelAvailable.length - 1))}
                    {availableItems.length === 0 && filteredModelAvailable.length === 0 && searchQuery && (
                  ...
                  <Card>
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 500, color: ws.body, fontFamily: f }}>Add custom endpoint</span>
                      </div>
                      <ChevronRight size={14} color={ws.disabled} />
                    </div>
                  </Card>
            ...
            {/* ── Category tabs (Apps/Data/Services) ────────────── */}
            {isCategoryTab && (
              <>
                <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ paddingBottom: 80 }}>
                  <SectionLabel>Available</SectionLabel>
                  <Card>
                    {availableItems.map((item, i) => renderRow(item, i === availableItems.length - 1))}
            ...
            {isModelsTab && (
              <>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <Card>
                    {filteredModelAvailable.map((item, i) => renderModelRow(item, i === filteredModelAvailable.length - 1))}
                    {filteredModelAvailable.length === 0 && searchQuery && (
                      <div style={{ padding: "20px 16px", textAlign: "center" }}>
      ...
      <DetailPanelShell open={panelOpen} onClose={handleClosePanel} isMobile={false} isDesktop={true} dataAttr="int-panel" fading={panelFading} top={84}>
