...
export default function DataTab() {
  ...
  function renderRow(item: IntegrationItem, last: boolean) {
    ...
    return (
      <ListRow key={item.id} onClick={() => handleSelectItem(item.id)} selected={isSelected} last={last} padding={isMobile ? "14px 14px" : "10px 14px"} onMouseEnter={() => setHoveredRowId(item.id)} onMouseLeave={() => setHoveredRowId(null)}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
